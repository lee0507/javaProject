...
public class P7_2 {//클래스 선언
	...
	public static void main(String[] args) throws IOException {//메인으로부터 프로그램시작, 예외가 발생하면 해당 클래스에서 벗어나게 된다.
		...
		File k30_f1 = new File("d:\\\\day_data\\\\Stock20150112.csv");//기존의 파일이나 폴더에 대한 제어를 하기 위해서 java.io 패키지의 File 클래스를 생성한다.
		...
		while ((k30_readtxt = k30_br.readLine()) != null) {//while 반복문의 조건을 (k30_readtxt = k30_br.readLine()) != null로 설정하여 읽어올 것이 없을때까지 계속해서 반복한다.
			...
			if(k30_field.length > 2 && k30_field[1].equals("20150112")) {//만약에 필드의 길이가 2보다 크고 field[2]의 문자가 20150112이라면

...
//굴림체로 출력하기
	public static void main(String[] args) {//메인으로부터 프로그램시작
		...
		// 불리안형 배열 k30_taxfree을 정의하고 값으로 물품의 면세여부를 저장한다.
		...
		for (int k30_i = 0; k30_i < k30_name.length; k30_i++) {//for반복문을 k30_i는 0부터 k30_i이 k30_itemname.length보다 작을 때까지 k30_i를 1씩 증가시키면서 수행한다.
			...
			if (k30_taxfree[k30_i] == true) {//만약에 k30_taxfree[k30_i]가 true라면(면세가 되는 제품이라면)
				...
				System.out.printf(" %s%9s%3s%10s\n", k30_temp, k30_df.format(k30_price[k30_i]), k30_amount[k30_i], k30_df.format(k30_amount[k30_i] * k30_price[k30_i]));
				//변수 k30_temp, k30_df.format(k30_price[k30_i]), k30_amount[k30_i], k30_df.format(k30_amount[k30_i] * k30_price[k30_i]와 줄바꿈을 화면에 출력한다.
				k30_taxsum += (k30_amount[k30_i] * k30_price[k30_i]);// 변수_taxfreesum에 k30_amount[k30_i]과 k30_price[k30_i] 곱한 값을 더해서 저장한다.
			}
			k30_count++;//제품이 5번나올때마다 줄을 출력하기 위해서 제품을 출력하는 반복문이 끝날때 변수 k30_count를 1씩 증가시킨다.
			if(k30_count % 5 == 0) {//만약에 k30_count를 5로 나누었을때 나머지가 0이라면(5번째 마다)
				System.out.printf("-----------------------------------------\n");//구분할 수 있는 선과 줄바꿈을 화면에 출력한다.
			}
		}
